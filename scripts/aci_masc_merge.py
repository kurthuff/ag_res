...
def load_data(year: int):
    ...
    return masc_df, aci_df, lut_df


def main():
    parser = argparse.ArgumentParser(description="Merge ACI proportions with MASC yield data by RM and Label.")
    parser.add_argument("--year", type=int, required=True)
    args = parser.parse_args()
    year = args.year

    masc_df, aci_df, lut_df = load_data(year)
    ...
    masc_df = masc_df.drop(columns=[lut_crop_col])
    ...
    df["yield_tonnes_per_acre_muni"] = df["yield_tonnes_per_acre_muni"].fillna(0)
    df["yield_total_muni"] = df["yield_total_muni"].fillna(0)
